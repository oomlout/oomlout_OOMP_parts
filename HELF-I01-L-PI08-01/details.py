import OOMP 
import OOMPtags 

######  Auto translated oomp file

newPart = OOMP.oompItem()
newPart.addTag('oompID','HELF-I01-L-PI08-01')
newPart.addTag('name','2.54 mm 8 Pin Blue Female Header (Long)')
newPart.addTag('hexID','HFL08L')
newPart.addTag('oompSort','')
newPart.addTag('oompClass','Through Hole')
newPart.addTag('oompClassCode','THTH')
newPart.addTag('oompType','HELF')
newPart.addTag('oompSize','I01')
newPart.addTag('oompColor','L')
newPart.addTag('oompDesc','PI08')
newPart.addTag('oompIndex','01')
newPart.addTag('oompVersion','40')
newPart.addTag('ooDesignator','J1')
newPart = OOMPtags.addTags(newPart,"HELF-I01-L-PI08-01")
OOMP.parts.append(newPart)
